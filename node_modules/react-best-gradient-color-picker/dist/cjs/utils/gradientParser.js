...
const gradientParser = (input = '') => {
    ...
    function matchHexColor() {
        ...
        if (hexObj?.value) {
            const { r, g, b, a } = (0, tinycolor2_1.default)(hexObj?.value).toRgb();
            return {
                value: `rgba(${r}, ${g}, ${b}, ${a})`,
            };
        }
    ...
    const checkCaps = (val) => {
        const capIt = (0, utils_js_1.isUpperCase)(val?.[0]);
        return {
    ...
    function matchLiteralColor() {
        ...
        if (litObj?.value) {
            const { r, g, b, a } = (0, tinycolor2_1.default)(litObj?.value).toRgb();
            return {
                value: `rgba(${r}, ${g}, ${b}, ${a})`,
            };
        }
    ...
    function getAST() {
        ...
        const ast0 = ast[0];
        const checkSelected = ast0?.colorStops?.filter((c) => (0, utils_js_1.isUpperCase)(c.value)).length;
        const getGradientObj = () => {
            if (checkSelected > 0) {
                return ast0;
            }
            else {
                const val = (c, i) => (i === 0 ? (0, formatters_js_1.high)(c) : (0, formatters_js_1.low)(c));
                return {
                    ...ast0,
    ...
    const convertHsl = (val) => {
        ...
        const { r, g, b, a } = (0, tinycolor2_1.default)({
            h: hsl[0],
            s: hsl[1],
            l: hsl[2],
            a: hsl[3] || 1,
